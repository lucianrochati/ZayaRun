...
14. **Autorregulação por CONSENTIMENTO (não impõe).** Decisão de produto: mexer no treino sem pedir é afronta/gera
    churn — autonomia sustenta aderência. `Profile.coach_autonomy` = `suggest` (padrão: só sugere, atleta decide) |
    `auto` (ajusta e avisa) | `off`. `CoachSuggestion` guarda a proposta; `accept_suggestion` aplica, `decline_suggestion`
